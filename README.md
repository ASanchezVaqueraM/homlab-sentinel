...
- [4. Requisitos Previos y Entorno](#4-requisitos-previos-y-entorno)
- [5. Estructura del Proyecto y Ubicación de Ficheros](#5-estructura-del-proyecto-y-ubicación-de-ficheros)
- [6. Implementación Técnica Paso a Paso](#6-implementación-técnica-paso-a-paso)
  - [6.1 Acceso Remoto Seguro y Redundancia (WireGuard + Tailscale)](#61-acceso-remoto-seguro-y-redundancia-wireguard--tailscale)
  - [6.2 Bastionado Criptográfico SSH (Claves Ed25519)](#62-bastionado-criptográfico-ssh-claves-ed25519)
  - [6.3 Pipeline de Alertas y Scripting (Telegram Integration)](#63-pipeline-de-alertas-y-scripting-telegram-integration)
  - [6.4 Integración de Fail2ban con Telegram](#64-integración-de-fail2ban-con-telegram)
  - [6.5 Monitorización Contenerizada con Docker Compose](#65-monitorización-contenerizada-con-docker-compose)
- [7. Guía de Despliegue Rápido (Quick Start)](#7-guía-de-despliegue-rápido-quick-start)
- [8. Pruebas de Validación y Verificación](#8-pruebas-de-validación-y-verificación)
- [9. Comandos Útiles de Mantenimiento y Diagnóstico](#9-comandos-útiles-de-mantenimiento-y-diagnóstico)
- [10. Consideraciones de Seguridad y Buenas Prácticas](#10-consideraciones-de-seguridad-y-buenas-prácticas)
- [11. Hoja de Ruta y Próximos Pasos (Roadmap)](#11-hoja-de-ruta-y-próximos-pasos-roadmap)
...
## 4. Requisitos Previos y Entorno

### 🍓 Hardware Recomendado
* **Host:** Raspberry Pi 4B (mínimo 2GB RAM, recomendado 4GB).
* **Almacenamiento:** Disco SSD USB 3.0 / NVMe vía adaptador (recomendado sobre tarjetas MicroSD para alta durabilidad de lecturas/escrituras en logs).
* **Alimentación:** Fuente oficial Raspberry Pi 5V 3A USB-C.

### 🌐 Red y Conectividad
* **IP Local Estática:** Reserva DHCP asignada en el router para la Raspberry Pi (`192.168.1.X`).
* **Port Forwarding:** Reenvío del puerto **UDP 51820** desde el router hacia la IP privada de la Pi.
* **Dominio Dinámico:** Subdominio gratuito en [DuckDNS](https://www.duckdns.org) asociado a la IP pública dinámica del hogar.

### 🤖 Telegram Bot
* Un Bot de Telegram creado mediante `@BotFather` para obtener el `TELEGRAM_BOT_TOKEN`.
* Identificador de Chat (`TELEGRAM_CHAT_ID`) obtenido mediante `@userinfobot`.

---

## 5. Estructura del Proyecto y Ubicación de Ficheros

A continuación se muestra la organización de los archivos en el repositorio y su ubicación correspondiente en el sistema operativo del nodo centinela:

```text
homelab-sentinel/
├── README.md
├── docker-compose.yml                          # Stack de Uptime Kuma
├── etc/
│   ├── fail2ban/
│   │   ├── action.d/
│   │   │   └── telegram.conf                   # Definición de acción de baneo a Telegram
│   │   └── jail.local                          # Configuración de Jaill SSH para Fail2ban
│   ├── profile.d/
│   │   └── ssh-telegram-alert.sh               # Interceptor de inicio de sesión SSH
│   ├── ssh/
│   │   └── sshd_config                         # Bastionado criptográfico de OpenSSH
│   ├── systemd/
│   │   └── system/
│   │       ├── telegram-boot.service           # Servicio systemd de arranque
│   │       └── ssh-failed-watcher.service      # Servicio systemd centinela de logs
│   └── wireguard/
│       └── wg0.conf                            # Configuración de servidor WireGuard
└── usr/
    └── local/
        └── bin/
            ├── telegram-notify                 # Conector HTTPS modular con Telegram
            └── ssh-failed-watcher.sh           # Daemon analizador de journalctl
```
...
### 6.1 Acceso Remoto Seguro y Redundancia (WireGuard + Tailscale)
...
### 6.2 Bastionado Criptográfico SSH (Claves Ed25519)
...
### 6.3 Pipeline de Alertas y Scripting (Telegram Integration)
...
*Asignación de permisos de ejecución y seguridad:*
```bash
...
### 6.4 Integración de Fail2ban con Telegram
...
### 6.5 Monitorización Contenerizada con Docker Compose
...
## 7. Guía de Despliegue Rápido (Quick Start)

Si deseas desplegar la configuración en una Raspberry Pi recién instalada, ejecuta la siguiente secuencia de comandos:
...
# 2. Copiar scripts ejecutables y aplicar permisos
sudo cp usr/local/bin/* /usr/local/bin/
sudo chmod 700 /usr/local/bin/telegram-notify
sudo chmod +x /usr/local/bin/ssh-failed-watcher.sh

# 3. Configurar tus credenciales de Telegram
sudo nano /usr/local/bin/telegram-notify

# 4. Desplegar configuraciones de sistema y servicios
sudo cp etc/profile.d/ssh-telegram-alert.sh /etc/profile.d/
sudo cp etc/systemd/system/*.service /etc/systemd/system/
sudo cp etc/fail2ban/action.d/telegram.conf /etc/fail2ban/action.d/
sudo cp etc/fail2ban/jail.local /etc/fail2ban/

# 5. Cargar servicios en systemd y reiniciar Fail2ban
sudo systemctl daemon-reload
sudo systemctl enable --now telegram-boot.service
sudo systemctl restart fail2ban

# 6. Levantar contenedor de Uptime Kuma
docker compose up -d
```

---

## 8. Pruebas de Validación y Verificación
...
## 9. Comandos Útiles de Mantenimiento y Diagnóstico

### 🛡️ Fail2ban & Seguridad
```bash
# Consultar el estado del jail SSH y lista de IPs baneadas
sudo fail2ban-client status sshd

# Desbanear manualmente una IP específica
sudo fail2ban-client set sshd unbanip <IP_OBJETIVO>
```

### 📡 Red & VPNs
```bash
# Verificar estado y tráfico de pares WireGuard
sudo wg show

# Comprobar estado de Tailscale y nodos de la malla
tailscale status
```

### 📜 Inspección de Logs en Tiempo Real
```bash
# Ver los últimos logs del daemon SSH
journalctl -u ssh -n 50 -f

# Probar envío de notificación directa por Telegram
/usr/local/bin/telegram-notify "🧪 <b>Prueba Manual de Mensaje</b>"
```
...
## 11. Hoja de Ruta y Próximos Pasos (Roadmap)

- [ ] **Automatización del Despliegue:** Creación de Playbooks de Ansible para aprovisionar el nodo perimetral de forma totalmente declarativa.
- [ ] **Telemetría de Recursos:** Integración de **Prometheus** + **Node Exporter** + **Grafana** para monitorizar temperatura de CPU, uso de RAM y salud de disco.
- [ ] **Backups Automatizados:** Configuración de **Restic / BorgBackup** para respaldar periódicamente las configuraciones de Pi-hole, Uptime Kuma y claves VPN hacia un almacenamiento remoto cifrado.
- [ ] **Servicios HTTP/HTTPS Expuestos:** Integración de **Cloudflare Tunnels** (*cloudflared*) para publicar servicios web sin necesidad de abrir puertos en el router.
