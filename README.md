...
Este proyecto documenta el diseño, fortificación (*hardening*) e implementación de un ecosistema de monitorización proactiva y seguridad perimetral para un nodo doméstico basado en **Raspberry Pi 4B**. El propósito central es garantizar la operatividad continua, trazabilidad inmediata de eventos de red y resistencia frente a vectores de ataque por fuerza bruta o accesos no autorizados, permitiendo una gestión desatendida y segura[cite: 1].

El sistema canaliza telemetría crítica e incidentes de seguridad en tiempo real hacia un bot privado de **Telegram**, integrando monitorización pasiva a nivel de kernel/servicios y monitorización activa de conectividad local e Internet mediante contenedores Docker[cite: 1].

---

## 2. Arquitectura del Sistema[cite: 1]
              ┌────────────────────────────────────────────────────────┐
...
## 3. Matriz de Componentes y Stack Tecnológico[cite: 1]
...
| **Uptime Kuma** | Monitorización continua de red | Sondas ICMP/DNS de baja sobrecarga con alertas directas integradas vía webhook Telegram. |[cite: 1]

---

## 4. Implementación Técnica Paso a Paso[cite: 1]

### 4.1. Hardening Criptográfico SSH (Claves Ed25519)[cite: 1]

Se descarta el uso tradicional de contraseñas para erradicar ataques de fuerza bruta y diccionarios automatizados[cite: 1]:

1. **Generación de pares de claves asimétricas en clientes:**[cite: 1]
   ```bash
   ssh-keygen -t ed25519 -C "alvaro-dispositivo"
   ```[cite: 1]
2. **Despliegue de la clave pública en el servidor (`~/.ssh/authorized_keys`):**[cite: 1]
   ```bash
   mkdir -p ~/.ssh && chmod 700 ~/.ssh
   echo "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5... alvaro-dispositivo" >> ~/.ssh/authorized_keys
   chmod 600 ~/.ssh/authorized_keys
   ```[cite: 1]
3. **Bloqueo en `/etc/ssh/sshd_config`:**[cite: 1]
   ```ini
   ...
   X11Forwarding no
   ```[cite: 1]
4. **Reinicio seguro tras comprobación de sintaxis:**[cite: 1]
   ```bash
...
### 4.2. Conector Central de Notificaciones (`telegram-notify`)[cite: 1]

Script modular en `/usr/local/bin/telegram-notify` con permisos `755`[cite: 1]:
...
### 4.3. Notificación de Sesiones SSH Exitosas[cite: 1]

Ubicado en `/etc/profile.d/ssh-telegram-alert.sh` para interceptar toda sesión interactiva abierta[cite: 1]:
...
### 4.4. Centinela de Intentos de Acceso Fallidos (`ssh-failed-watcher`)[cite: 1]

Servicio en streaming para interceptar contraseñas incorrectas o usuarios inexistentes en tiempo real[cite: 1]:

* **Script ejecutable (`/usr/local/bin/ssh-failed-watcher.sh`):**[cite: 1]
  ```bash
  #!/bin/bash
...
[cite: 1]
Unidad Systemd de tipo oneshot (/etc/systemd/system/telegram-boot.service) disparada tras confirmar enlace de red[cite: 1]:
Ini, TOML
[Unit]
...
### 4.6. Integración de Fail2ban con Telegram[cite: 1]

1. **Acción de bloqueo (`/etc/fail2ban/action.d/telegram.conf`):**[cite: 1]
   ```ini
   [Definition]
   ...
   ```[cite: 1]
2. **Jail SSH en `/etc/fail2ban/jail.local`:**[cite: 1]
   ```ini
   [DEFAULT]
   bantime  = 1h
   findtime = 10m
...
### 4.7. Monitorización Continua con Uptime Kuma en Docker[cite: 1]

Despliegue contenerizado y persistente[cite: 1]:
...
* **Sondas configuradas:**[cite: 1]
  * **PC Sobremesa:** Monitor ICMP Ping cada 60 s hacia la IP interna de la LAN[cite: 1].
  * **DNS / Pi-hole:** Sonda DNS hacia `127.0.0.1:53` verificando resolución de dominios en tiempo y forma[cite: 1].
  * **Internet Gateway / WAN:** Monitor ICMP Ping hacia `1.1.1.1` para detección de caídas de fibra o corte de ISP[cite: 1].
* **Canal de alerta:** Webhook nativo a la API de Telegram con el mismo bot y chat ID[cite: 1].

---

## 5. Pruebas de Validación y Verificación[cite: 1]
...
| **Caída de Host / Servicio** | Apagado de equipo sobremesa o desconexión DNS | Notificación Push de caída emitida por Uptime Kuma | ✅ Superado |[cite: 1]

---

## 6. Consideraciones de Seguridad y Buenas Prácticas[cite: 1]

1. **Gestión de Secretos:** El archivo `/usr/local/bin/telegram-notify` contiene el token del bot; debe pertenecer a `root:root` con permisos `700` o `750` para evitar lecturas por usuarios sin privilegios[cite: 1].
2. **Rotación y Auditoría de Claves:** Para dar de baja un cliente comprometido o extraviado, basta con eliminar su línea en `~/.ssh/authorized_keys`[cite: 1].
3. **Huella Criptográfica:** Cada clave pública autorizada puede ser verificada mediante su huella digital ejecutando[cite: 1]:
   ```bash
...
## 7. Licencia y Autoría[cite: 1]

* **Autor:** Proyecto Personal de Seguridad y Administración de Sistemas[cite: 1]
* **Licencia:** MIT License[cite: 1]
